...
int main(int argc, char *argv[]) {
  ...
  for(i=0;i<imagesize;i++) {
    ...
    // this little piece of code thresholds out very green pixels
    // the resulting image will be very green where there is green stuff
    ...
    if(j > 10) {
      image[i].g = image[i].r < 128 ? image[i].r*2 : image[i].r;
    }
    else {
      image[i].g = min;
    }
