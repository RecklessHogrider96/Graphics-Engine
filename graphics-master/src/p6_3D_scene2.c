...
int main(int argc, char *argv[])
{
  ...
  module_module(scene, tieFighter);
  ...
  for (int i = 0; i < frames; i++)
  {
    ...
    matrix_rotateY(&gtm, cos(i * 1 * M_PI / 36.0), sin(i * 2 * M_PI / 36.0));
    module_draw(scene, &vtm, &gtm, ds, NULL, src);

    sprintf(filename, "p6_3D_scene2_%04d.ppm", i);
  ...
  system("convert -delay 10 p6_3D_scene2_*.ppm ../images/3DScene2.gif");
  system("rm -f p6_3D_scene2_*");
